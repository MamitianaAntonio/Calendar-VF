...
const ScheduleClass = () => {
  ...
  return (
    <div>
      ...
      <button onClick={handleAddClass}>Ajouter le Cours</button>

      <h3>Liste des Cours Planifiés</h3>
      <ul>
        {classes.map((cls) => (
          <li key={cls.id}>
            Professeur ID: {cls.professor_id}, Matière ID: {cls.subject_id}, Début: {cls.start_time}, Fin: {cls.end_time}, Durée: {cls.duration}h
          </li>
        ))}
      </ul>
    </div>
